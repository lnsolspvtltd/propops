...
app.include_router(approvals.router)  # has own prefix /api/v1/approvals

# Phase 3 routes
app.include_router(auth_routes.router)  # /api/v1/auth

# Phase 2 routes (registered when those PRs are merged)
try:
    ...
    logger.info("Phase 2 routes not yet available (merge Phase 2 PRs): %s", e)

# Demo routes (development only)
try:
    from backend.core.config import get_settings as _gs
    if _gs().environment == "development":
        from backend.api.routes import demo
        app.include_router(demo.router)
        logger.info("Demo routes registered (development mode)")
except Exception as e:
    logger.debug("Demo routes not loaded: %s", e)
